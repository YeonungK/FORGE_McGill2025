...
int servoTest = 10;
int servoTestRight = 11;

Servo testServo;
Servo testServoRight;

//for left button servo
bool servoActive = false;

unsigned long servoStartTime = 0;

const int servoDuration = 500;

//for right button servo
bool servoActiveRight = false;    // New variable for right button servo state
unsigned long servoStartTimeRight = 0; // Timer for right button
const int servoDurationRight = 500;
...
void setup() {
  ...
  testServo.attach(servoTest);
  testServo.write(0);

  testServoRight.attach(servoTestRight);
  testServoRight.write(0);
...
void loop() {
  ...
  if (bValueL == LOW && !servoActive) {
    servoActive = true;               // Mark the servo as active
    servoStartTime = millis();        // Start the timer
    testServo.write(90);              // Move the servo to position 90 degrees
    Serial.println("Left button pressed, servo moving...");
}

// If the servo is active and the timer reaches the specified duration
if (servoActive && millis() - servoStartTime >= servoDuration) {
    testServo.write(0);              // Return the servo to its initial position
    servoActive = false;             // Mark the servo as inactive
    Serial.println("Servo returned to starting position.");
}

// Same logic for the right button
if (bValueR == LOW && !servoActiveRight) {  // Check if the right button is pressed
  servoActiveRight = true;                   // Mark the servo as active for the right button
  servoStartTimeRight = millis();            // Start the timer for right button press
  testServoRight.write(90);                       // Move the servo to position 90 degrees
  Serial.println("Right button pressed, servo moving...");
}

// If the servo is active and the timer reaches the specified duration for right button
if (servoActiveRight && millis() - servoStartTimeRight >= servoDurationRight) {
  testServoRight.write(0);                       // Return the servo to its initial position
  servoActiveRight = false;                 // Mark the servo as inactive
  Serial.println("Servo returned to starting position.");
}
...
/*
  if (button1.isPressed()) {
    ...
    testServo.write(180);              // tell servo to go to position in variable 'pos'
    delay(500);
    testServo.write(0);                       // waits 15 ms for the servo to reach the position
}
*/
...
//  Serial.println("  " + Rcom_dir);
  ...
  if (Lcom_dir == 0) { // when the left joystick is not moved
    ...
    if (Rcom_dir == 1 || Rcom_dir == 5 || Rcom_dir == 9) { // turn left
      ...
      //RmotorBspeed = 0;
      //RmotorFspeed = 110;
      LmotorBspeed = 150; //was 0
      LmotorFspeed = 0; //was 125
      RmotorBspeed = 0;
      RmotorFspeed = 125; //-25
  ...
  if (Lcom_dir >= 4 && Lcom_dir <= 6) { // when the left joystick is moving up
    ...
    LmotorFspeed = 180;
    RmotorBspeed = 0;
    RmotorFspeed = 205; //was 185
    if (Rcom_dir == 1 || Rcom_dir == 5 || Rcom_dir == 9) { // turn left
      LmotorBspeed = 0;
      LmotorFspeed -= 50; //was 20
      RmotorBspeed = 0;
      RmotorFspeed += 50; //was 30
    }
    else if (Rcom_dir == 2 || Rcom_dir == 6 || Rcom_dir == 10) { // turn right
      LmotorBspeed = 0;
      LmotorFspeed += 50;
  ...
  writeMotorSpeed(LmotorFspeed, LmotorBspeed, RmotorBspeed, RmotorFspeed, Rcom_dir, bValueL);
...
void writeMotorSpeed(int LmotorFspeed, int LmotorBspeed, int RmotorBspeed, int RmotorFspeed, int Rcom_dir, int bValueL) {
